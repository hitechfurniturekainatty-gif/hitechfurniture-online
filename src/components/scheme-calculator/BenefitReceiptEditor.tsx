...
import { monthRef, refId, refLabel, targetCatalog, allAttributedReceipts } from "./schemeAttribution";
...
export function BenefitReceiptEditor({ receipts, onChange, rules: localRules = [], defaultMonth, allowMonthChoice = false, schemeMonths = [], schemePeriods = [], defaultPeriod }: { schemeMonths?: VendorMonth[]; schemePeriods?: PeriodBenefitRecord[]; defaultPeriod?: SchemePeriodRef; defaultMonth?: string; allowMonthChoice?: boolean; receipts: BenefitReceipt[]; onChange: (receipts: BenefitReceipt[]) => void; rules?: SettlementRule[] }) {
  ...
  const source = draft.scheme_period || defaultPeriod || monthRef(defaultMonth || draft.date || new Date().toISOString());
  const rules = targetCatalog(schemeMonths,schemePeriods).find(g=>refId(g.ref)===refId(source))?.rules || (schemeMonths.length ? [] : localRules);
  const matchingReceipts = schemeMonths.length ? allAttributedReceipts(schemeMonths,schemePeriods).filter(r=>refId(r.source)===refId(source)) : receipts;
  ...
  return <section className="benefit-entry space-y-3 rounded-xl border bg-background/60 p-4">
    <div><h5 className="text-sm font-semibold">Additional benefits · അധിക ആനുകൂല്യങ്ങൾ</h5><p className="mt-1 text-xs text-muted-foreground">Invoice-ന് പുറത്തായി ലഭിച്ച credit note / cashback / free item മാത്രം ഇവിടെ ചേർക്കുക. Invoice-ൽ രേഖപ്പെടുത്തിയ benefit വീണ്ടും ചേർക്കേണ്ടതില്ല.</p></div>
    ...
    <SchemePeriodPicker value={source} onChange={scheme_period=>patch({scheme_period,scheme_rule_key:undefined,replaces_free_qty:undefined})}/>
