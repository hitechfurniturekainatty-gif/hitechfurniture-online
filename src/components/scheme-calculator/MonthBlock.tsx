...
function itemAwareReport(rows: Row[], fallback: { kind: SchemeKind; config: any }) {
  ...
  const rep: any[] = [], targets: any[] = []; let weightedPct = 0, qtyWeight = 0;
  for (const g of groups.values()) {
    if (g.kind === "bogo" && Array.isArray(g.config?.rules) && !g.config.rules.some((r: any) => String(r?.purchaseItem || "").trim())) continue;
    const agg = aggregateRowsByItem(g.rows); const report: any = computeFreeReport({ kind: g.kind, config: g.config }, agg);
    ...
    const qty = g.rows.reduce((s, r) => s + (Number(r.qty) || 0), 0); weightedPct += computeAchievementPct({ kind: g.kind, config: g.config }, agg) * qty; qtyWeight += qty;
  }
...
export function MonthBlock({ vm, fy, savedSchemes, onChange, onSave }: { vm: VendorMonth; fy: number; savedSchemes: SchemeRow[]; onChange: (patch: Partial<VendorMonth>) => void; onSave: () => void | Promise<void>; }) {
 ...
 const totalQty=flatRows.reduce((s,r)=>s+(+r.qty||0),0),totalAmount=flatRows.reduce((s,r)=>s+(+r.amountWithTax||0),0),totalMrp=flatRows.reduce((s,r)=>s+(+r.mrp||0)*(+r.qty||0),0); const vendorDiscount=totalMrp>0?Math.max(0,(totalMrp-totalAmount)/totalMrp*100):0; const free=report.rep.reduce((s:number,r:any)=>s+(+r.free||0),0),targets=report.targets as any[]; const completion=report.completion; const label=`${MONTH_NAME[vm.month]} ${fyCalendarYear(fy,vm.month)}`;
 ...
 return <div className={`rounded-2xl border bg-card shadow-sm ${isCurrent?"border-primary/50":"border-border"}`}>
  ...
  {open&&<div className="space-y-4 border-t p-4">
   ...
   <section className="admin-section-card border-dashed p-4"><div className="mb-3 flex flex-wrap justify-between gap-2"><div><h4 className="font-semibold">② All invoices for {label}</h4><p className="text-xs text-muted-foreground">Keep every invoice item. MRP is saved vendor-wise and exact item matches can reuse it automatically. Only configured scheme items are counted for scheme eligibility.</p></div><div className="flex flex-wrap gap-3"><Stat label="Invoices" value={String(invoices.length)}/><Stat label="MRP Value" value={`₹${fmt(totalMrp)}`}/><Stat label="Cost incl. Tax" value={`₹${fmt(totalAmount)}`}/><Stat label="Vendor Discount" value={totalMrp>0?`${fmt(vendorDiscount)}%`:"Add MRP"}/><Stat label="Eligible Free" value={fmt(free)}/></div></div><Button size="sm" onClick={()=>{setDialogInvoice({id:crypto.randomUUID(),label:`Invoice ${invoices.length+1}`,rows:[]});setDialogOpen(true)}}><Plus className="h-4 w-4"/> Add Invoice</Button><div className="mt-3 space-y-3">{invoices.map((inv,i)=><InvoiceCard key={inv.id} index={i} invoice={inv} savedSchemes={savedSchemes} fallbackScheme={{kind:vm.scheme_kind,config:vm.scheme_config}} onChange={p=>updateInvoice(inv.id,p)} onRemove={()=>setInvoices(invoices.filter(x=>x.id!==inv.id))} onEdit={()=>{setDialogInvoice(inv);setDialogOpen(true)}}/>)}</div></section>
   <InvoiceDialog open={dialogOpen} invoice={dialogInvoice} partyId={vm.party_id} onClose={()=>setDialogOpen(false)} onSave={inv=>{const exists=invoices.some(x=>x.id===inv.id);setInvoices(exists?invoices.map(x=>x.id===inv.id?inv:x):[...invoices,inv]);setDialogOpen(false)}}/>
