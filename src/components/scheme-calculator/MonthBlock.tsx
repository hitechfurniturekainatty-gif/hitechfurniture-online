...
export function MonthBlock({ vm, fy, savedSchemes, onChange, onSave }: { vm: VendorMonth; fy: number; savedSchemes: SchemeRow[]; onChange: (patch: Partial<VendorMonth>) => void; onSave: () => void | Promise<void>; }) {
 ...
 const free=report.rep.reduce((s:number,r:any)=>s+(+r.free||0),0),targets=report.targets as any[]; const completion=report.completion; const label=`${MONTH_NAME[vm.month]} ${fyCalendarYear(fy,vm.month)}`;
