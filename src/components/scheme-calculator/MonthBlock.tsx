...
export function MonthBlock({ vm, fy, savedSchemes, onChange, onSave }: { vm: VendorMonth; fy: number; savedSchemes: SchemeRow[]; onChange: (patch: Partial<VendorMonth>) => void; onSave: () => void | Promise<void>; }) {
 ...
 useEffect(()=>{const untouchedLegacyDefault=flatRows.length===0&&vm.scheme_kind==="company"&&Number(vm.scheme_config?.everyQty)===10;if(untouchedLegacyDefault)onChange({scheme_kind:"bogo",scheme_config:defaultConfig("bogo")});},[vm.month,vm.scheme_kind]);
 ...
 return <div className={`rounded-2xl border-2 bg-card shadow-sm ${isCurrent?"border-primary/50":"border-border"}`}>
  ...
  {open&&<div className="space-y-4 border-t p-4">
   ...
   <section className="rounded-xl border bg-background/50 p-4"><h4 className="mb-3 font-semibold">③ Scheme result</h4>{flatRows.length===0?<p className="text-sm text-muted-foreground">Add an invoice to start live scheme analysis.</p>:<div className="grid gap-4 lg:grid-cols-3"><div className="rounded-xl border-2 border-emerald-500/30 bg-emerald-500/5 p-4"><div className="flex gap-3"><ProgressRing pct={completion} size={72}/><div><div className="flex items-center gap-1 text-emerald-700"><CheckCircle2 className="h-4 w-4"/><b>Eligible Free: {free}</b></div><div className="mt-1 text-xs text-muted-foreground">Calculated from matched scheme items across all invoices.</div></div></div></div><div className="rounded-xl border bg-background p-4"><div className="text-xs text-muted-foreground">Received Free</div><div className="mt-1 text-2xl font-semibold text-emerald-600">{fmt(receivedFree)}</div></div><div className="rounded-xl border bg-background p-4"><div className="text-xs text-muted-foreground">Pending Free</div><div className="mt-1 text-2xl font-semibold">{fmt(pendingFree)}</div></div>{targets.length>0&&<div className="rounded-xl border-2 border-amber-500/40 bg-amber-500/5 p-4 lg:col-span-3"><div className="flex items-center gap-1 text-amber-700"><AlertTriangle className="h-4 w-4"/><b className="text-sm">Next target</b></div><div className="mt-2 grid gap-2 md:grid-cols-2">{targets.slice(0,6).map((t:any,i:number)=><div key={i} className="rounded-lg border bg-background/70 p-2 text-xs"><b>Buy {fmt(t.gap)} more</b> → {t.reward}<div className="text-muted-foreground">{t.item}</div></div>)}</div></div>}</div>}</section>
