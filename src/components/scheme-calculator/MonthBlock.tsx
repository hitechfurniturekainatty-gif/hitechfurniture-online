...
import { Stat } from "./Stat"; import { ProgressRing } from "./ProgressRing"; import { SchemeConfigEditor } from "./SchemeConfigEditor"; import { InvoiceCard } from "./InvoiceCard"; import { InvoiceDialog } from "./InvoiceDialog"; import { ItemBenefitTracker } from "./ItemBenefitTracker";
...
function itemAwareReport(rows: Row[], fallback: { kind: SchemeKind; config: any }) {
  ...
  for (const g of groups.values()) {
    // New quantity schemes are opt-in item rules. A blank rule means no scheme,
    // not "apply Buy X/Get Y to every invoice line".
...
export function MonthBlock({ vm, fy, savedSchemes, onChange, onSave }: { vm: VendorMonth; fy: number; savedSchemes: SchemeRow[]; onChange: (patch: Partial<VendorMonth>) => void; onSave: () => void | Promise<void>; }) {
 ...
 useEffect(()=>{const untouchedLegacyDefault=flatRows.length===0&&vm.scheme_kind==="company"&&Number(vm.scheme_config?.everyQty)===10;if(untouchedLegacyDefault)onChange({scheme_kind:"bogo",scheme_config:exactQuantityConfig()});},[vm.month,vm.scheme_kind]);
 ...
 const receivedFree=(vm.benefit_receipts||[]).filter(r=>r.kind==="free_item").reduce((s,r)=>s+(Number(r.qty)||0),0); const pendingFree=Math.max(0,free-receivedFree);
 ...
 const simpleKinds:SchemeKind[]=["bogo","percent"]; const visibleKinds=simpleKinds.includes(vm.scheme_kind)?simpleKinds:[vm.scheme_kind,...simpleKinds];
 return <div className={`rounded-2xl border bg-card shadow-sm ${isCurrent?"border-primary/50":"border-border"}`}>
  <button onClick={()=>setOpen(!open)} className="flex w-full items-center gap-4 px-4 py-3 text-left"><div className="admin-accent-tile admin-accent-sage flex h-12 w-12 flex-col items-center justify-center rounded-xl"><span className="text-[9px]">{MONTH_NAME[vm.month]}</span><b className="text-xs">{String(fyCalendarYear(fy,vm.month)).slice(-2)}</b></div><div className="flex-1"><div className="font-display">{label} {isCurrent&&<span className="ml-2 rounded-full bg-primary px-2 py-0.5 text-[10px] text-primary-foreground">CURRENT</span>}</div><div className="text-xs text-muted-foreground">{totalQty} qty · ₹{fmt(totalAmount)} · <span className="text-emerald-600">{free} eligible</span> · {receivedFree} received · <b>{pendingFree} pending</b></div></div><b>{completion}%</b>{open?<ChevronUp/>:<ChevronDown/>}</button>
  {open&&<div className="space-y-4 border-t p-4">
   <section className="admin-section-card p-4"><div className="mb-2 flex justify-between"><div><h4 className="text-sm font-semibold">① Scheme items for this month</h4><p className="text-xs text-muted-foreground">Add only the item that actually has a scheme. Exact item is default. Family / contains works only when you choose it.</p></div>{savedSchemes.length>0&&<Select value="" onValueChange={applySaved}><SelectTrigger className="h-8 w-[200px]"><SelectValue placeholder="Apply template…"/></SelectTrigger><SelectContent>{savedSchemes.map(s=><SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent></Select>}</div><div className="mb-3 max-w-md"><Label className="text-xs">Scheme type</Label><Select value={vm.scheme_kind} onValueChange={v=>{const kind=v as SchemeKind;onChange({scheme_kind:kind,scheme_config:freshConfig(kind)})}}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{visibleKinds.map(k=><SelectItem key={k} value={k}>{k==="bogo"?"Item-wise Quantity Scheme":k==="percent"?"Percentage Discount":SCHEME_LABEL[k]}</SelectItem>)}</SelectContent></Select></div><SchemeConfigEditor scheme={{kind:vm.scheme_kind,config:vm.scheme_config}} onChange={c=>onChange({scheme_config:c})}/></section>
   <section className="admin-section-card border-dashed p-4"><div className="mb-3 flex flex-wrap justify-between gap-2"><div><h4 className="font-semibold">② All invoices for {label}</h4><p className="text-xs text-muted-foreground">Keep every invoice item. Only configured scheme items are counted. Variants are combined only for a rule where you selected Family / contains.</p></div><div className="flex gap-3"><Stat label="Invoices" value={String(invoices.length)}/><Stat label="Cost" value={`₹${fmt(totalAmount)}`}/><Stat label="Eligible Free" value={fmt(free)} tone="success"/></div></div><Button size="sm" onClick={()=>{setDialogInvoice({id:crypto.randomUUID(),label:`Invoice ${invoices.length+1}`,rows:[]});setDialogOpen(true)}}><Plus className="h-4 w-4"/> Add Invoice</Button><div className="mt-3 space-y-3">{invoices.map((inv,i)=><InvoiceCard key={inv.id} index={i} invoice={inv} savedSchemes={savedSchemes} fallbackScheme={{kind:vm.scheme_kind,config:vm.scheme_config}} onChange={p=>updateInvoice(inv.id,p)} onRemove={()=>setInvoices(invoices.filter(x=>x.id!==inv.id))} onEdit={()=>{setDialogInvoice(inv);setDialogOpen(true)}}/>)}</div></section>
   ...
   <section className="admin-section-card p-4"><h4 className="mb-3 font-semibold">③ Scheme result</h4>{flatRows.length===0?<p className="text-sm text-muted-foreground">Add an invoice to start live scheme analysis.</p>:<div className="grid gap-4 lg:grid-cols-3"><div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-4"><div className="flex gap-3"><ProgressRing pct={completion} size={72}/><div><div className="flex items-center gap-1 text-emerald-700"><CheckCircle2 className="h-4 w-4"/><b>Eligible Free: {free}</b></div><div className="mt-1 text-xs text-muted-foreground">Calculated only from the scheme item rules you entered.</div></div></div></div><div className="rounded-xl border bg-background p-4"><div className="text-xs text-muted-foreground">Received Free</div><div className="mt-1 text-2xl font-semibold text-emerald-600">{fmt(receivedFree)}</div></div><div className="rounded-xl border bg-background p-4"><div className="text-xs text-muted-foreground">Pending Free</div><div className="mt-1 text-2xl font-semibold">{fmt(pendingFree)}</div></div>{targets.length>0&&<div className="rounded-xl border border-amber-500/40 bg-amber-500/5 p-4 lg:col-span-3"><div className="flex items-center gap-1 text-amber-700"><AlertTriangle className="h-4 w-4"/><b className="text-sm">Next target</b></div><div className="mt-2 grid gap-2 md:grid-cols-2">{targets.slice(0,6).map((t:any,i:number)=><div key={i} className="rounded-lg border bg-background/70 p-2 text-xs"><b>Buy {fmt(t.gap)} more</b> → {t.reward}<div className="text-muted-foreground">{t.item}</div></div>)}</div></div>}</div>}</section>
