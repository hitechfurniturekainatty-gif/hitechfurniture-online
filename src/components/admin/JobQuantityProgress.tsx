...
export const JobQuantityProgress = ({
  ...
  const load = async () => {
    ...
    const { data, error } = await db
      ...
      .select("id, assigned_qty, completed_qty, quotation_item_id, quotation_items(description, quantity, ready_stock_qty)")
      .eq("job_id", jobId)
    ...
    <div className="space-y-2 rounded-lg border border-border/60 bg-muted/20 p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <p className="text-xs font-medium">
          Ready {totals.completed}/{totals.assigned} · Pending {Math.max(0, totals.assigned - totals.completed)}
        </p>
      </div>
            ...
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  Assigned {row.assigned_qty} · Completed {row.completed_qty} · Pending {pending}
                </p>
              </div>
              {row.item && (
