...
import { CalendarClock, IndianRupee, Loader2, Phone } from "lucide-react";

const db=supabase as any;
type Row={id:string;customer_name:string|null;phone:string|null;place:string|null;pending_amount:number;next_follow_up_at:string|null};
...
export function ReceivablesTodayPanel(){
 ...
 useEffect(()=>{(async()=>{const{data}=await db.from("receivables").select("id,customer_name,phone,place,pending_amount,next_follow_up_at").is("closed_at",null).gt("pending_amount",0).not("next_follow_up_at","is",null).order("next_follow_up_at",{ascending:true}).limit(50);setRows(data??[]);setLoading(false)})()},[]);
 ...
 return <section className="space-y-3"><div className="flex flex-wrap items-end justify-between gap-2"><div><h2 className="font-display text-xl font-semibold">Collections Today</h2><p className="text-xs text-muted-foreground">Customers whose promised payment/follow-up date is today or overdue.</p></div><Button asChild size="sm" variant="outline"><Link to="/admin/backlog">Open Receivables</Link></Button></div><div className="grid gap-2 sm:grid-cols-3"><Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">Call Today / Overdue</p><p className="text-2xl font-bold">{due.length}</p></CardContent></Card><Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">Overdue Calls</p><p className="text-2xl font-bold">{overdue.length}</p></CardContent></Card><Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">Balance in Follow-up</p><p className="text-2xl font-bold">{money(total)}</p></CardContent></Card></div>{due.length>0&&<Card><CardContent className="divide-y p-0">{due.slice(0,5).map(r=><div key={r.id} className="flex flex-wrap items-center gap-3 p-3"><div className="flex-1"><div className="flex flex-wrap gap-1"><Badge variant={r.next_follow_up_at&&key(new Date(r.next_follow_up_at))<today?"destructive":"default"}>{r.next_follow_up_at&&key(new Date(r.next_follow_up_at))<today?"OVERDUE":"CALL TODAY"}</Badge></div><p className="mt-1 font-medium">{r.customer_name||"Customer"}</p><p className="text-xs text-muted-foreground">{[r.place,r.phone].filter(Boolean).join(" · ")}</p></div><div className="text-right"><p className="text-xs text-muted-foreground">Balance to Receive</p><p className="font-semibold">{money(Number(r.pending_amount))}</p></div>{r.phone&&<Button asChild size="icon" variant="outline"><a href={`tel:+91${r.phone}`}><Phone className="h-4 w-4"/></a></Button>}<Button asChild size="sm" variant="outline"><Link to={`/admin/backlog?receivable=${r.id}`}><CalendarClock className="mr-1 h-4 w-4"/>Open</Link></Button></div>)}</CardContent></Card>}{due.length===0&&<p className="rounded-lg border p-4 text-center text-sm text-muted-foreground">No receivable calls due today.</p>}</section>;
}
