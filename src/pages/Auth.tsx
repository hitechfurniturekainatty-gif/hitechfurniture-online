...
const Auth = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "forgot") {
        ...
        toast({ title: "Reset link sent", description: "Use only the newest reset email and open the link once." });
        return;
      }

      if (mode === "reset") {
        if (recoveryChecking) throw new Error("Please wait while the secure reset link is verified.");
        ...
        if (error) throw error;
        toast({ title: "Password updated", description: "Your password is ready. Sign in with the new password." });
      ...
      if (mode === "signup") {
        ...
        const creds = useWorker ? { email: phoneToEmail(identifier), password: pinToPassword(password) } : { email: identifier.trim().toLowerCase(), password };
        const { error } = await supabase.auth.signInWithPassword(creds);
        if (error) throw error;
        if (next) { window.location.href = next; return; }
        const { data: userData } = await supabase.auth.getUser();
        const uid = userData.user?.id;
        let displayName = (userData.user?.user_metadata as any)?.display_name || userData.user?.email?.split("@")[0] || "there";
        if (uid) {
          ...
          else if (isOffice || roles.includes("measurement_staff")) navigate("/admin/my-work");
  ...
  const description = mode === "login" ? "Staff use email & password. Workers use phone & PIN." : mode === "signup" ? "First account becomes admin automatically." : mode === "forgot" ? "Enter your staff email. We’ll send a secure password reset link." : recoveryChecking ? "Verifying your secure reset link…" : recoveryReady ? "Enter a new password for your account." : "This reset link is not active. Request a fresh reset email.";

  return (
    <div className="min-h-screen bg-gradient-to-br from-secondary/40 to-background flex flex-col">
      <header className="container-page py-6"><Link to="/"><Logo className="h-10 w-10" /></Link></header>
      <main className="flex flex-1 items-center justify-center px-4 py-10">
        <Card className="w-full max-w-md shadow-elegant border-border/60">
          <CardHeader><CardTitle className="font-display text-2xl">{title}</CardTitle><CardDescription>{description}</CardDescription></CardHeader>
          <CardContent>
