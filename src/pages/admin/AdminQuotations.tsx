...
const AdminQuotations = () => {
  ...
  const docFiltered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (!isPO(t) && r.salesperson_name === "Website Enquiry" && stageFor(r).stage === 1) return false;
  ...
  const staffOptions = useMemo(() => {
    ...
    rows.forEach((r) => {
      const t: DocType = (r.document_type as DocType) ?? "quotation";
      if (t !== docTab) return;
      if (!isPO(t) && r.salesperson_name === "Website Enquiry" && stageFor(r).stage === 1) return;
  ...
  const docCounts = useMemo(() => {
    ...
    const matchesSearch = (r: Q) =>
      ...
      r.party_place.toLowerCase().includes(s);
    return {
      quotation: rows.filter((r) => ((r.document_type as DocType) ?? "quotation") === "quotation" && !(r.salesperson_name === "Website Enquiry" && stageFor(r).stage === 1) && matchesSearch(r)).length,
