...
const AdminOverview = () => {
  ...
  const showAdmin=isAdmin,showOffice=isOfficeStaff,showProduction=isOfficeStaff,showWarehouse=isOfficeStaff||isWarehouse,showDelivery=isOfficeStaff||isDelivery;
  ...
  return <AdminShell><div className="mb-6 sm:mb-8"><h1 className="font-display text-2xl sm:text-3xl">Furniture Shop Command Center</h1><p className="mt-1 text-sm text-muted-foreground sm:text-base">One place for today's work — enquiry, custom orders, production, dispatch, delivery and customer collections.</p></div>{sections.length===0?<p className="text-muted-foreground">No dashboard sections are available for your role yet. Ask an admin to check your access.</p>:<div>{sections.map(s=><div key={s.key} className="border-t border-border/60 py-8 first:border-t-0 first:pt-0">{s.node}</div>)}</div>}</AdminShell>;
};
