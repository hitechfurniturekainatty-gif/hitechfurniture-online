...
BEGIN
  ...
  SET commercial_status = CASE
        ...
        WHEN NOT _has_ordered AND _has_open THEN 'quote_preparation'
...
-- Backfill item conversion state for orders confirmed before item-level conversion tracking existed.
WITH legacy AS (
  SELECT qi.id, qi.quotation_id, qi.quantity, qi.ordered_qty, qi.conversion_status
  FROM public.quotation_items qi
  JOIN public.quotations q ON q.id=qi.quotation_id
  WHERE q.deleted_at IS NULL
    AND q.status IN ('finalized','delivered')
    AND q.commercial_status IN ('confirmed','payment_pending','closed')
    AND COALESCE(qi.ordered_qty,0)=0
    AND COALESCE(qi.cancelled_qty,0)=0
)
UPDATE public.quotation_items qi
SET ordered_qty=legacy.quantity,
    conversion_status='ordered',
    converted_at=COALESCE(qi.converted_at,now())
FROM legacy
WHERE qi.id=legacy.id;

-- Full confirmation converts only quantities that are still open.
-- Previously-cancelled quantities remain cancelled and are never resurrected.
...
BEGIN
  ...
  IF _q.commercial_status='confirmed'
     AND NOT EXISTS (
       SELECT 1 FROM public.quotation_items
       WHERE quotation_id=_quotation_id
         AND COALESCE(ordered_qty,0)+COALESCE(cancelled_qty,0) < quantity
